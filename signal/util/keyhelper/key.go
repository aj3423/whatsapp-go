...
func GeneratePreKeys(start int, count int) ([]*record.PreKey, error) {
	...
	for i := start; i <= count; i++ {
		...
		preKeys = append(preKeys, record.NewPreKey(uint32(i), key))
	}
...
// of the install.
func GenerateLastResortKey() (*record.PreKey, error) {
	keyPair, err := ecc.GenerateKeyPair()
	if err != nil {
		return nil, err
	}
	return record.NewPreKey(0, keyPair), nil
}
...
func GenerateSignedPreKey(identityKeyPair *identity.KeyPair, signedPreKeyID uint32) (*record.SignedPreKey, error) {
	...
	signature, e := ecc.CalculateSignature(identityKeyPair.PrivateKey(), keyPair.PublicKey().Serialize())
	if e != nil {
		return nil, e
	}
	timestamp := time.Now().Unix()

	return record.NewSignedPreKey(signedPreKeyID, timestamp, keyPair, signature), nil
}
