...
func NewKeyFromStruct(structure *pb.SessionStructure_Chain_ChainKey, kdf kdf.HKDF) *Key {
	return NewKey(
		kdf,
		structure.Key,
		structure.GetIndex(),
...
func NewStructFromKey(key *Key) *pb.SessionStructure_Chain_ChainKey {
	return &pb.SessionStructure_Chain_ChainKey{
		Key:   key.key,
		Index: proto.Uint32(key.index),
	}
}
