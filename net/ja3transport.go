...
func ExtMap() map[string]tls.TLSExtension {
	return map[string]tls.TLSExtension{
		...
		"16": &tls.ALPNExtension{
			...
			AlpnProtocols: []string{"http/1.1"}, // disable "h2" for WA
		},
		"18": &tls.SCTExtension{},
...
func NewTransport(ja3 string) (*fhttp.Transport, error) {
	return NewTransportWithConfig(ja3, &tls.Config{})
}

// NewTransportWithConfig creates an http.Transport object given a utls.Config
func NewTransportWithConfig(ja3 string, config *tls.Config) (*fhttp.Transport, error) {
	...
	return &fhttp.Transport{DialTLS: dialtls}, nil
}

// NewTransportWithConfig creates an http.Transport object given a utls.Config
func NewTransportWithConfigAndSocks5(ja3 string, s5_url string, config *tls.Config) (*fhttp.Transport, error) {
	...
	dialtls := func(network, addr string) (net.Conn, error) {
		// !!!!!!!!!!!!!!!!
		// https://stackoverflow.com/questions/60624954/connect-via-proxy-while-using-utls-and-http-1-1-request/64282413#64282413
		...
		if proxyURI.Scheme != "socks5" {
			return nil, errors.New("aj unsupported proxy scheme: " + proxyURI.Scheme)
		}
...
func stringToSpec(ja3 string) (*tls.ClientHelloSpec, error) {
	...
	if len(tokens) != 5 {
		return nil, errors.New("invalid ja3: tokens.len != 4: '" + ja3 + "'")
	}
